...
const InvoiceListview = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`https://office3i.com/api/public/api/proforma_invoice_view_list/${id}`, {
                    headers: {
    ...
    return (
        <div className="container mt-4 mb-8" style={{ width: '210mm', height: '297mm' }}>
            <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '20px 20px', gap:'5px' }}>
                <Button className='mb-2'>
                    Pay Now!
                </Button>

                <button className="btn btn-primary mb-2" onClick={handlePrint}>Print Invoice</button>
            </div>
