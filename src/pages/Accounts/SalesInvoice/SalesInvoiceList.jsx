...
const useremail = customerData?.useremail || '';
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch('https://office3i.com/api/public/api/view_saleinvoice', {
                method: 'GET',
                headers: {
...
'Registered-Email': officialuseremail,
    ...
    const handleDelete = async (id) => {
        try {
            ...
            if (reason) {
                const response = await fetch('https://office3i.com/api/public/api/delete_saleinvoice', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${usertoken}`,
