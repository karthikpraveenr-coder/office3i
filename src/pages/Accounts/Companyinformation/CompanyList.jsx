...
const useremail = customerData?.useremail || '';
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await fetch('https://office3i.com/api/public/api/view_company_information', {
...
'Registered-Email': officialuseremail,
    ...
    const handleDelete = async (id) => {
        try {
            ...
            if (reason) {
                const response = await fetch('https://office3i.com/api/public/api/delete_company_information', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${usertoken}`,
