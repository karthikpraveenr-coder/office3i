...
const officialuseremail = customerData?.officialuseremail || '';
    ...
    useEffect(() => {
        ...
        axios.get(`https://office3i.com/api/public/api/editview_purchaseinvoice/${id}`, {
            headers: {
    ...
    const handleSave = async (e) => {
        ...
        try {
            const response = await fetch('https://office3i.com/api/public/api/updatepurchaseinvoice', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${usertoken}`,
                    'Registered-Email': officialuseremail
    ...
    useEffect(() => {
        ...
        fetch('https://office3i.com/api/public/api/sales_company_list', {
            headers: {
                'Authorization': `Bearer ${usertoken}`,
                'Registered-Email': officialuseremail
            }
    ...
    //     fetch('https://office3i.com/api/public/api/autogeneratesaleinvoiceid', {
    //         headers: {
    //             'Authorization': `Bearer ${usertoken}`,
    // 'Registered-Email': officialuseremail
    //         }
    ...
    useEffect(() => {
        fetch('https://office3i.com/api/public/api/sales_item_list', {
            headers: {
                'Authorization': `Bearer ${usertoken}`,
                'Registered-Email': officialuseremail
            }
    ...
    useEffect(() => {
        const fetchHsnSacOptions = async () => {
            ...
            for (const [index, item] of items.entries()) {
                if (item.descriptionalGoods) {
                    try {
                        const response = await fetch(`https://office3i.com/api/public/api/sales_hsn_sac/${item.descriptionalGoods}`, {
                            headers: {
                                'Authorization': `Bearer ${usertoken}`,
                                'Registered-Email': officialuseremail
                            }
    ...
    const handleChange = async (index, field, value) => {
        ...
        if (field === 'descriptionalGoods') {
            ...
            if (selectedOption) {
                try {
                    const response = await fetch(`https://office3i.com/api/public/api/sales_hsn_sac/${value}`, {
                        headers: {
                            'Authorization': `Bearer ${usertoken}`,
                            'Registered-Email': officialuseremail
                        }
        ...
        <div className="container mt-5" style={{ padding: '0px 70px 0px' }}>
            ...
            <div style={{ boxShadow: '#0000007d 0px 0px 10px 1px', padding: '35px 50px' }}>
                <form onSubmit={handleSave}>
                    ...
                    <button type="submit" className="btn btn-primary" style={{ marginRight: '10px' }}>Update Purchase Invoice</button>
