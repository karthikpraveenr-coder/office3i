...
const useremail = customerData?.useremail || '';
    ...
    useEffect(() => {
        const fetchAssetId = async () => {
            try {
                const response = await axios.get(`https://office3i.com/api/public/api/edit_asset_typelist/${id}`, {
                    headers: {
...
'Registered-Email': officialuseremail // Assuming usertoken is defined somewhere
    ...
    const handleSave = (e) => {
        ...
        axios.put('https://office3i.com/api/public/api/update_asset_type', requestData, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${usertoken}`,
