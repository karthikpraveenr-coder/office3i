...
const EditAssets = () => {
    ...
    useEffect(() => {
        const fetchAssetId = async () => {
            try {
                const response = await axios.get(`https://office3i.com/api/public/api/edit_assign_assetlist/${id}`, {
                    headers: {
                ...
                if (response.data.status === 'success') {
                    ...
                    const employeeArray = response.data.data.asset_id ? response.data.data.asset_id.split(',').map(member => member.trim()) : [];
    ...
    const handleSave = async (e) => {
        ...
        const formData = {
            ...
            remarks: remarks,
            status: status,
            updated_by: customerData.userempid
        };

        try {
            const response = await fetch('https://office3i.com/api/public/api/update_assign_asset', {
                method: 'PUT',
    ...
    useEffect(() => {
        const fetchDepartments = async () => {
            try {
                const response = await axios.get('https://office3i.com/api/public/api/userrolelist', {
                    headers: {
                        Authorization: `Bearer ${usertoken}`,
                        'Registered-Email': officialuseremail
                    }
    ...
    useEffect(() => {
        if (department) {
            const apiUrl = `https://office3i.com/api/public/api/employee_dropdown_list/${department}`;
    ...
    const [selectedAssetType, setSelectedAssetType] = useState([]); // Initialize as an array

    useEffect(() => {
        const fetchAssetTypes = async () => {
            try {
                const response = await axios.get('https://office3i.com/api/public/api/asset_name', {
                    headers: {
                        'Authorization': `Bearer ${usertoken}`,
                        'Registered-Email': officialuseremail
                    }
                ...
                const data = response.data.data || [];
                setAssetTypeDropdown(data); // Set the fetched asset types
            } catch (error) {
    ...
    // Format the asset dropdown options for the MultiSelect component
    const formattedAssetTypeDropdown = assetTypeDropdown.map(asset => ({
        label: asset.asset_type_name,
        value: asset.id.toString() // Ensure the value is a string
    }));

    // Handle selection change in MultiSelect
    const handleSelectAssetTypeChange = (selectedOptions) => {
        const selectedIds = selectedOptions.map(option => option.value); // Extract selected option values
        setSelectedAssetType(selectedIds); // Update state with selected IDs
    };

    // Format selected asset types to match MultiSelect expected format
    const formattedSelectedAssetType = selectedAssetType.map(selectedId => {
        const foundOption = formattedAssetTypeDropdown.find(option => option.value === selectedId);
        return foundOption ? { label: foundOption.label, value: foundOption.value } : null;
                            ...
                            <Row className="mb-3">
                                <Col md={6}>
                                    <Form.Group controlId="formAssetType">
                                        ...
                                        <MultiSelect
                                            options={formattedAssetTypeDropdown} // Dropdown options
                                            value={formattedSelectedAssetType} // Pass formatted selected values
                                            onChange={handleSelectAssetTypeChange} // Handle change event
                                            labelledBy="Select"
                                        />
                                    </Form.Group>
