...
const useremail = customerData?.useremail || '';
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch('https://office3i.com/api/public/api/assign_assetlist', {
        method: 'POST',
        headers: {
...
'Registered-Email': officialuseremail
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (reason) {
        const response = await fetch('https://office3i.com/api/public/api/delete_assign_asset', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${usertoken}`,
