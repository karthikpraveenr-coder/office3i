...
const useremail = customerData?.useremail || '';
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch('https://office3i.com/api/public/api/view_assigntask_list', {
...
'Registered-Email': officialuseremail,
    ...
    const handleDelete = async (id) => {
        try {
            ...
            if (reason) {
                const response = await fetch('https://office3i.com/api/public/api/delete_task', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${usertoken}`,
'Registered-Email': officialuseremail // Assuming usertoken is defined somewhere
    ...
    const applyFilter = async (e) => {
        ...
        try {
            const response = await fetch('https://office3i.com/api/public/api/filter_tasklist', {
                method: 'POST',
                body: formData,
                headers: {
                    'Authorization': `Bearer ${usertoken}`,
...
'Registered-Email': officialuseremail
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const apiUrl = `https://office3i.com/api/public/api/employee_dropdown_list/${formattedSelectedDepartment}`;
            try {
...
'Registered-Email': officialuseremail
    ...
    const handleModalSubmit = async (e) => {
        ...
        try {
            const response = await fetch('https://office3i.com/api/public/api/task_update_status', {
                method: 'PUT',
                                        ...
                                        return (
                                            <tr key={row.id}>
                                                ...
                                                <td className='no-print'>{row.attachment !== null ?
                                                    <button style={{ padding: '6px' }} className="btn-view" onClick={() => { window.open(`https://office3i.com/api/storage/app/${row.attachment}`, '_blank') }}>
