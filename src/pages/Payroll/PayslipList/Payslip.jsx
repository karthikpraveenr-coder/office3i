...
const useremail = customerData?.useremail || '';
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await fetch(`https://office3i.com/api/public/api/get_emp_payslip_list/${id}`, {
...
'Registered-Email': officialuseremail,
    ...
    const handleDelete = async (id) => {
        try {
            ...
            if (reason) {
                const response = await fetch('https://office3i.com/api/public/api/delete_payslip_list', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${usertoken}`,
