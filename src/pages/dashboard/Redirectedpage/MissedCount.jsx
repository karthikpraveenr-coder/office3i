...
const useremail = customerData?.useremail || '';
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch('https://office3i.com/api/public/api/missedcount_Details', {
                method: 'GET',
                headers: {
...
'Registered-Email': officialuseremail,
    ...
    const fetchindividualcustomerData = async () => {
        setLoading(true);
        try {
            const response = await fetch(`https://office3i.com/api/public/api/get_missedcount_Details/${missedcounuserid}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${usertoken}`,
'Registered-Email': officialuseremail,
            ...
            if (response.ok) {
                ...
                if (responseData.status === 'success' && responseData.data.length > 0) {
                    console.log("fetchindividualcustomerData", responseData.data[0].role);
