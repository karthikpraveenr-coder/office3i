...
const useremail = customerData?.useremail || '';
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await fetch('https://office3i.com/api/public/api/view_raiseassign_list', {
                method: 'POST',
                headers: {
...
'Registered-Email': officialuseremail
    ...
    const handleDelete = async (id) => {
        try {
            ...
            if (reason) {
                const response = await fetch('https://office3i.com/api/public/api/delete_raiseticket', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${usertoken}`,
                                        ...
                                        return (
                                            <tr key={row.id}>
                                                ...
                                                <td className='no-print'>
                                                    {row.attachment !== '-' ?
                                                        <button className="btn-view" onClick={() => { window.open(`https://office3i.com/api/storage/app/${row.attachment}`, '_blank') }}>
