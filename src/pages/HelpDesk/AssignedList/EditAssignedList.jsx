...
const useremail = customerData?.useremail || '';
    ...
    useEffect(() => {
        if (attachment) {
            document.querySelector("#img").setAttribute("src", `https://office3i.com/api/storage/app/${attachment}`);
        }
    }, [attachment]);
    ...
    useEffect(() => {
        const fetchAssetId = async () => {
            try {
                const response = await axios.get(`https://office3i.com/api/public/api/editview_raiselist/${id}`, {
                    headers: {
...
'Registered-Email': officialuseremail // Assuming usertoken is defined somewhere
    ...
    const handleSave = (e) => {
        ...
        axios.put('https://office3i.com/api/public/api/update_raiseticket', formData, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${usertoken}`,
'Registered-Email': officialuseremail
    ...
    useEffect(() => {
        if (assignDepartment) {
            const apiUrl = `https://office3i.com/api/public/api/employee_dropdown_list/${assignDepartment}`;
            const fetchEmployees = async () => {
                try {
                    const response = await axios.get(apiUrl, {
                        headers: {
    ...
    useEffect(() => {
        const fetchDepartments = async () => {
            try {
                const response = await axios.get('https://office3i.com/api/public/api/issue_type_list', {
                    headers: {
                        Authorization: `Bearer ${usertoken}`,
                        'Registered-Email': officialuseremail
                    }
